...
* Github 활용이 미흡했다.
  * project issue 작성
  * 업무 세부 사항 작성 및 분담
  * 같은 파일을 여러 이름 폴더 하위에 보관하기
* 계층형 아키텍쳐 이해가 부족했다.
  * 인스턴스/메소드/ 클래스 개념 이해가 부족하다는 것을 느꼈다.
* 무조건 코드를 작성하는 것이 중요한게 아니란 것을 느꼈다.
* 업무 계획을 자세하고 간결하고 세부적으로 작성해야 나중에 코드 작성할 때도 업무 분업에 편하다는 것을 느꼈다.

#### 🔥앞으로 각오🔥
* GItHub의 성능을 믿고 이름 폴더 하위가 아닌 프로젝트 파일 하나로 팀원들과 교류하며 작업 할 것이다.
* 계층형 아키텍쳐 부분 공부를 혼자 작은 프로젝트를 한다고 가정하며 더 많이 만들어보고 공부해봐야겠다.
* 프로젝트가 들어가면 제일 먼저 프로젝트에 어떤 업무가 필요하고 어떤 클래스/메소드/인스턴스가 필요한지 세부적으로 팀원들과 상의하며 WBS 와 github issue 를 작성하고  업무를 세부적으로 작성 후 적절하게 분담하여 프로젝트를 진행 할 것이다.

**더 열심히 공부해서 다음 프로젝트 때는 제 몫을 다하는 팀원이 되자..!!**
